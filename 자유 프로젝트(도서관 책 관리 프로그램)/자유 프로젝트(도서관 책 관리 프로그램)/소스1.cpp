...
Book* books = NULL; // 도서 포인터로 변경
Member* members = NULL; // 회원 포인터로 변경
...
int main() {
    ...
    while (1) { // 메뉴 출력
        ...
        case 0: saveData(); // 프로그램 종료 시 데이터 저장
            printf("프로그램 종료\n");
            free(books); // 동적 메모리 해제
            free(members); // 동적 메모리 해제
            return 0;
...
// 도서 등록 함수
void addBook() {
    if (bookCount >= 100) { // 도서가 최대 수에 도달했을 경우
        printf("도서가 가득 찼습니다.\n");
        return;
    }
    ...
    // 도서 배열에 새로운 도서 추가
    books = (Book*)realloc(books, (bookCount + 1) * sizeof(Book)); // 메모리 재할당
...
// 도서 조회 함수
void viewBooks() {
    if (bookCount == 0) { // 등록된 도서가 없을 경우
        printf("등록된 도서가 없습니다.\n");
        return;
    }
    printf("\n도서 목록:\n");
    for (int i = 0; i < bookCount; i++) { // 도서 정보 출력
        printf("ID: %d, 제목: %s, 저자: %s, 대출 가능: %s\n", books[i].id, books[i].title, books[i].author, books[i].available ? "예" : "아니오");
    }
}
...
void borrowBook() {
    ...
    if (memberIndex == -1) { // 회원이 존재하지 않으면 오류 출력
    ...
    if (bookIndex == -1) { // 도서가 존재하지 않으면 오류 출력
        printf("도서가 존재하지 않습니다.\n");
        return;
    }

    if (books[bookIndex].available == 0) { // 도서가 대출 중일 경우
        printf("도서가 대출 중입니다.\n");
        return;
    }
...
void returnBook() {
    ...
    if (members[memberIndex].borrowedBookId == -1) { // 해당 회원이 대출한 도서가 없으면 오류 출력
...
// 회원 등록 함수
void addMember() {
    if (memberCount >= 100) { // 회원이 최대 수에 도달했을 경우
        printf("회원이 가득 찼습니다.\n");
        return;
    }
    ...
    // 회원 배열에 새로운 회원 추가
    members = (Member*)realloc(members, (memberCount + 1) * sizeof(Member)); // 메모리 재할당
...
// 회원 조회 함수
void viewMembers() {
    if (memberCount == 0) { // 등록된 회원이 없을 경우
        printf("등록된 회원이 없습니다.\n");
        return;
    }

    printf("\n회원 목록:\n");
    for (int i = 0; i < memberCount; i++) { // 회원 정보 출력
        printf("ID: %d, 이름: %s, 대출 도서 ID: %d\n", members[i].id, members[i].name, members[i].borrowedBookId == -1 ? 0 : members[i].borrowedBookId);
    }
}
